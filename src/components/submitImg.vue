  ...
  <div class="submitImg" id="submitImg">
    ...
    <div class="contentBox">
      ...
      <div class="rightContent">
        <div class="addNav">
          <!--提示信息-->
          <span class="hint" v-if="isActive!==0">最多可上传{{maxNumbe}}张图片<em v-if="isActive>1">,建议每次上传不超过10张!</em></span>
          <!--点击按钮-->
                ...
                <div class="compileContent" v-if="todo.picId==coveIndex&&isCompile==true"
                  ...
                  <p class="labelBox">
                    ...
                    <input type="text" class="longAndLat" v-model="temporaryCoordinate" maxlength="21"
                           placeholder="经度在前，纬度在后，英文逗号分开，小数点后六位"/>
                  </p>
                  <p class="labelBox">
  ...
  export default {
    ...
    methods: {
      ...
      register() {
        this.$http({
          method: 'post',
          url: 'longcity/services/v1/index/login',
          data: {"loginName": 'yening', "password": '888888'}
          //data:{"loginName":'test3', "password":'888888'}
        }).then(function (succeed) {
          console.log(succeed);
          localStorage.setItem("token", succeed.data.result.token);//登录成功保存token
        }, function (err) {
          console.log(err)
        });
      },
                  ...
                  if (that.M >= ImgSize) {
                    if (i <= files.length) {
                      ...
                      if (imgNumber <= that.maxNumbe && i + statistics <= that.maxNumbe) {
                        ...
                        that.hint("1最多可上传" + that.maxNumbe + "张图片!");
                      }
                    } else {
                      that.hint("2最多可上传" + that.maxNumbe + "张图片!");
                    }
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          for (var i = 0; i < that.AllDataList[0].picturesRef.length; i++) {
            if (that.AllDataList[0].picturesRef[i].picId == id) {
              ...
              if (that.AllDataList[0].picturesRef.length == 10) {
    ...
    mounted() {
      ...
      for(var i=0;that.theInitImgData.length;i++){
        that.theInitImgData[i].textTag=[];
        for(var j=0;j<that.theInitImgData[i].picTag.length;j++){
          for(var k=0;k<that.keywordList.length;k++){
